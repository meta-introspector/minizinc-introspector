...
constraint embedding_dimension > 1.0 /\ embedding_dimension < 3.0;  % Typical range for embeddings in
       networks
...
constraint embedding_dimension >= sum(i in index_set(godel_numbers_of_self)) (godel_numbers_of_self[i] mod
       100) / 100.0 + 1.0;
