...
const char* minizinc_get_version_string() {
    // MiniZinc::versionString() returns a std::string.
    // We need to return a C-style string that remains valid.
    // This is a simple approach, but the string will be leaked.
    // For a real application, proper memory management is needed.
    return strdup(MiniZinc::versionString().c_str());
}