...
${LIBMINIZINC_BUILD_DIR}/minizinc "${MINIZINC_MODELS_DIR}/generate_vector_params_v2.mzn" -D "num_vec=${NUM_VEC}; base_size=${BASE_SIZE};" | head -n -1 > "${VECTOR_PARAMS_DZN_FILE}"
...
MINIZINC_OUTPUT=$(eval "$MINIZINC_COMMAND" 2>&1)
...
echo "--- End Raw MiniZinc Output ---"

if [ $MINIZINC_EXIT_CODE -ne 0 ]; then
    echo "MiniZinc model run failed!"
    echo "$MINIZINC_OUTPUT" # Output full error log for debugging
    exit 1
else
