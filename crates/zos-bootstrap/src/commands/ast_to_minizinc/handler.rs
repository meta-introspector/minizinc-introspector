...
pub fn handle_ast_to_minizinc_command(args: AstToMiniZincArgs) -> crate::utils::error::Result<()> {
    ...
    let mut env = MiniZincEnvironment::new().map_err(|e| crate::utils::error::ZosError::MiniZincFfiError(format!("Failed to create MiniZinc environment: {}", e)))?;
    ...
    ).map_err(|e| crate::utils::error::ZosError::MiniZincFfiError(format!("Failed to generate MiniZinc model: {}", e)))?;
