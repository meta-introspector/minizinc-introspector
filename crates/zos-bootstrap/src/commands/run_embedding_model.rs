...
Relations Version: {{}}",
    ...
    fs::write(proof_tape_dir.join("version_vector.txt"), version_vector_content)?; // Initial write, will append vector_params_source
    ...
    let vector_params_file: PathBuf;
    match vector_params_source {
        VectorParamsSource::Version(version) => {
            vector_params_file = minizinc_data_dir.join(format!("example_vector_params_{}.dzn", version));
            fs::write(proof_tape_dir.join("version_vector.txt"), format!("Vector Params Source: Version({})", version))?;
            fs::copy(&vector_params_file, proof_tape_dir.join(vector_params_file.file_name().unwrap()))?;
        }
        VectorParamsSource::NumVec(num_vec) => {
            let rust_data_generator_exe = project_root
                .join("tools")
                .join("minizinc_data_generator_rs")
                .join("minizinc_data_generator_rs")
                .join("target")
                .join("release")
                .join("minizinc_data_generator_rs");
            let generated_file = proof_tape_dir.join("generated_vector_params.dzn");

            let generator_output = subprocess::run_command(
                &rust_data_generator_exe.to_string_lossy(),
                &[&num_vec.to_string()],
            )?;
            fs::write(&generated_file, &generator_output.stdout)?;
            vector_params_file = generated_file;
            fs::write(proof_tape_dir.join("version_vector.txt"), format!("Vector Params Source: NumVec({})", num_vec))?;
        }
    }

    // Copy common files to proof tape
    ...
    println!("Embedding model run completed. Proof tape in: {{}}", proof_tape_dir.to_string_lossy());
    Ok(())
}
