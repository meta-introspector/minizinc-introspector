...
pub async fn handle_capture_session_output_command(crq_number: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (session_name, pane_id) in sessions_and_panes {
        gemini_eprintln!("Processing session: {session_name}", session_name = session_name);
        gemini_eprintln!("Capturing pane: {pane_id} in session: {session_name}", pane_id = pane_id, session_name = session_name);
        ...
        gemini_eprintln!("Captured content from pane {session_name}:{pane_id} saved to {output_file_path}", session_name = session_name, pane_id = pane_id, output_file_path = output_file_path.display());
    }
