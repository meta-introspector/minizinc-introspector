...
pub fn handle_ast_to_minizinc_command(args: AstToMiniZincArgs) -> crate::zos_bootstrap_commands::utils::error::Result<()> {
    println!("\n--- Starting AST to MiniZinc Process ---");
    println!("Analyzing project: {}", args.project_root);

    let mut env = MiniZincEnvironment::new().map_err(|e| crate::zos_bootstrap_commands::utils::error::ZosError::MiniZincFfiError(format!("Failed to create MiniZinc environment: {}", e)))?;
    ...
    if target_index == 0 {
        return Err(crate::zos_bootstrap_commands::utils::error::ZosError::InvalidArgument("target_index must be 1-indexed (i.e., >= 1).".to_string()).into());
    }
    ...
    if let (Some(subset_index), Some(total_subsets)) = (args.file_subset_index, args.total_file_subsets) {
        if total_subsets == 0 {
            return Err(crate::zos_bootstrap_commands::utils::error::ZosError::InvalidArgument("total_file_subsets cannot be 0.".to_string()).into());
        }
        if subset_index >= total_subsets {
            return Err(crate::zos_bootstrap_commands::utils::error::ZosError::InvalidArgument("file_subset_index must be less than total_file_subsets.".to_string()).into());
        }
    ...
    if let (Some(subset_index), Some(total_subsets)) = (args.ast_element_subset_index, args.total_ast_element_subsets) {
        if total_subsets == 0 {
            return Err(crate::zos_bootstrap_commands::utils::error::ZosError::InvalidArgument("total_ast_element_subsets cannot be 0.".to_string()).into());
        }
        if subset_index >= total_subsets {
            return Err(crate::zos_bootstrap_commands::utils::error::ZosError::InvalidArgument("ast_element_subset_index must be less than total_ast_element_subsets.".to_string()).into());
        }
    ...
    ).map_err(|e| crate::zos_bootstrap_commands::utils::error::ZosError::MiniZincFfiError(format!("Failed to generate MiniZinc model: {}", e)))?;
    ...
    if args.plan_mode {
        ...
        if !output.status.success() {
            return Err(crate::zos_bootstrap_commands::utils::error::ZosError::CommandFailed {
