//! This module defines the data structures for configuring the `gemini-context`
//! subcommand of `mini-act`. It includes parameters for generating Gemini CLI
//! calls and for controlling the context sizing using MiniZinc.
...
/// Arguments for the `gemini-context` subcommand.
#[derive(Debug, Args)]
pub struct GeminiContextArgs {
    /// The task or query for the Gemini CLI.
    #[arg(long)]
    pub task: String,

    /// Desired context size in megabytes for MiniZinc optimization.
    #[arg(long)]
    pub context_size_mb: Option<u32>,

    /// Whether to include Git history in the context.
    #[arg(long)]
    pub include_git_history: bool,

    /// Glob patterns for files to include in the context.
    #[arg(long, value_delimiter = ',')]
    pub file_patterns: Vec<String>,

    /// Gemini CLI specific options.
    #[clap(flatten)]
    pub gemini_cli_options: GeminiCliOptions,
}
...
mod tests {
    ...
    #[test]
    fn test_gemini_context_args_creation() {
        let args = GeminiContextArgs {
            task: "Refactor code".to_string(),
            context_size_mb: Some(10),
            include_git_history: true,
            file_patterns: vec!["src/**/*.rs".to_string(), "tests/**/*.rs".to_string()],
            gemini_cli_options: GeminiCliOptions::default(),
        };

        assert_eq!(args.task, "Refactor code");
        assert_eq!(args.context_size_mb, Some(10));
        assert_eq!(args.include_git_history, true);
        assert_eq!(args.file_patterns.len(), 2);
        assert!(args.gemini_cli_options.model.is_none());
    }
