...
async fn fetch_workflow_runs(octocrab: &Octocrab, owner: &str, repo: &str) -> Result<Vec<Run>> {
    ...
    let page = octocrab.workflows(owner, repo).list_all_runs().send().await?;
    Ok(page.items)
}
...
async fn fetch_artifacts_for_run(octocrab: &Octocrab, owner: &str, repo: &str, run_id: u64) -> Result<Vec<WorkflowListArtifact>> {
    eprintln!("Fetching artifacts for run {} in {}/{}", run_id, owner, repo);
    let page = octocrab.actions().list_workflow_run_artifacts(owner.to_string(), repo.to_string(), RunId(run_id)).send().await?;
    Ok(page.value.unwrap().items)
}
...
async fn fetch_job_logs(octocrab: &Octocrab, owner: &str, repo: &str, job_id: u64) -> Result<String> {
    ...
    let response = octocrab.get::<String, _, _>(url, None::<&()>)
        .await?;
    Ok(response)
}
    ...
    match &cli.command {
        ...
        Commands::Jobs { run_id } => {
            ...
            for job in jobs {
                ...
                // job.steps is Vec<Step>, not Option<Vec<Step>>
                println!("  Steps:");
