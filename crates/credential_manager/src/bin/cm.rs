            ...
            match service {
                ImportService::Aws => {
                    ...
                    if aws_credentials_path.exists() {
                        let contents = fs::read_to_string(&aws_credentials_path)?;
                        ...
                        for line in contents.lines() {
                            ...
                            if line.starts_with('[') && line.ends_with(']') {
                                ...
                                if parts.len() == 2 {
                                    ...
                                    match key {
                                        "aws_access_key_id" => {
                                            store_credential("aws", &format!("{}_access_key_id", current_section), value)?;
                                            println!("Stored AWS access key for profile: {}", current_section);
                                        },
                                        "aws_secret_access_key" => {
                                            store_credential("aws", &format!("{}_secret_access_key", current_section), value)?;
                                            println!("Stored AWS secret key for profile: {}", current_section);
                                        },
                                        "aws_session_token" => {
                                            store_credential("aws", &format!("{}_session_token", current_section), value)?;
                                            println!("Stored AWS session token for profile: {}", current_section);
                                        },
                                        _ => {} // Ignore other keys
                                    }
                                }
                    ...
                    if aws_config_path.exists() {
                        let contents = fs::read_to_string(&aws_config_path)?;
                        let mut current_section = String::new();
                        for line in contents.lines() {
                            let line = line.trim();
                            if line.starts_with('[') && line.ends_with(']') {
                                ...
                                if parts.len() == 2 {
                                    ...
                                    let value = parts[1].trim();
                                    if key == "region" {
                                        println!("Found AWS region for profile {}: {}", current_section, value);
                                    }
                                }
                            }
                        }
