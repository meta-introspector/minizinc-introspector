...
static FUNCTION_REGISTRY: Lazy<HashMap<String, CallbackFn>> = Lazy::new(|| {
    ...
    for (_name, _callback_fn_ptr) in FUNCTIONS {
...
pub fn _handle_old_meme_regex(_line: &str, captures: Vec<String>, fixed_fm: &mut FixedFrontMatter) -> Result<(), anyhow::Error> {
...
//#[poem_macros::poem_function]
pub fn _handle_new_meme_desc_regex(_line: &str, captures: Vec<String>, fixed_fm: &mut FixedFrontMatter) -> Result<(), anyhow::Error> {
    fixed_fm.pending_meme_description = Some(captures[1].trim().to_string());
    Ok(())
}

//#[poem_macros::poem_function]
pub fn _handle_new_meme_template_regex(_line: &str, captures: Vec<String>, fixed_fm: &mut FixedFrontMatter) -> Result<(), anyhow::Error> {
